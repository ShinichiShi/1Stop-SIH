...
import {REDIS_HOST, REDIS_PORT, REDIS_USERNAME, REDIS_PASSWORD, MAX_START_RETRIES, MONGODB_URL, SERVER_PORT, START_RETRY_DELAY_MS} from "./config";
  ...
  while (retries < MAX_START_RETRIES) {
    try {
      ...
      if (retries === MAX_START_RETRIES) {
        console.error("Max retries reached. Server will continue without DB.");
        return;
      }
